...
def read_args(argv):
    ...
    parser.add_argument('--signature', help='path to the signagure', type=argparse.FileType('rb'), default=None)
...
def write_data(args, header):
    ...
    num_dt_pages = (header.unused[0] + header.page_size - 1) / header.page_size
    ...
    start_signature_addr = start_dt_addr + header.page_size * num_dt_pages
    signature_buf_len = os.fstat(args.bootimg.fileno()).st_size - start_signature_addr
    ...
    signature_buf = read_buf_data(args.signature, args.bootimg, start_signature_addr, signature_buf_len)
    ...
    write_buf_data(args.output, signature_buf)
